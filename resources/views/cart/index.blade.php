...
@section('title', 'Mi Carrito - Gracia Creativa')
        ...
        <div class="space-y-6">
            ...
            <div class="card-hakesa divide-y divide-gray-100">
                ...
                <div class="p-5 flex gap-4" data-cart-item>
                    <!-- Image -->
                    <div class="w-20 h-20 bg-gray-700 rounded-xl flex-shrink-0 overflow-hidden">
                    ...
                    <div class="flex-1 min-w-0">
                        <h3 class="font-bold text-white truncate">{{ $item->product->name }}</h3>
                        <p class="text-gracia-primary-dark font-semibold">₡{{ number_format($item->product->price, 0, ',', '.') }}</p>
                        @if($item->customization)
                            <p class="text-sm text-gray-400 mt-1 truncate">Personalización: {{ $item->customization }}</p>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center gap-3 flex-shrink-0"
                        ...
                        <div class="flex items-center gap-2">
                            <input type="number" name="quantity" value="{{ $item->quantity }}" min="1" max="{{ $item->product->stock }}"
                                ...
                                class="w-16 px-3 py-1.5 border border-gray-600 rounded-lg text-center text-sm focus:outline-none focus:ring-2 focus:ring-gracia-primary transition-shadow">
                            <!-- Loading indicator -->
                            <span x-show="updating" x-cloak class="text-gracia-primary">
                    ...
                    <div class="text-right flex-shrink-0">
                        <p class="font-bold text-white" data-subtotal>₡{{ number_format($item->subtotal, 0, ',', '.') }}</p>
                    </div>
                </div>
                @endforeach
            </div>
