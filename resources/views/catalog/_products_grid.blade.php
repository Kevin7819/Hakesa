    ...
    <div class="card-hakesa overflow-hidden group">
        <div class="relative">
            <a href="{{ route('catalog.show', $product) }}">
                ...
                @else
                    <div class="h-52 overflow-hidden bg-gradient-to-br from-hakesa-pink/20 to-hakesa-teal/20 flex items-center justify-center">
            ...
            <button x-data="wishlistToggle({{ $product->id }}, {{ in_array($product->id, $wlIds) ? 'true' : 'false' }})"
                ...
                class="absolute top-3 right-3 w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-md hover:bg-pink-50 transition-all disabled:opacity-50"
                aria-label="Agregar a favoritos">
                <svg x-show="!loading" class="w-5 h-5" :fill="inWishlist ? '#F26BB5' : 'none'" stroke="#F26BB5" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
