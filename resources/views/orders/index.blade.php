...
@section('title', 'Mis Pedidos - Gracia Creativa')
            ...
            <a href="{{ route('orders.show', $order) }}" class="card-hakesa p-5 block group">
                <div class="flex justify-between items-start">
                    ...
                    <div class="text-right">
                        <p class="font-bold text-lg text-white">₡{{ number_format($order->total, 0, ',', '.') }}</p>
                        <x-order-status :status="$order->status" />
                    </div>
                </div>
                <p class="text-sm text-gray-400 mt-2">{{ $order->items_count }} producto(s)</p>
            </a>
            @endforeach
        </div>
